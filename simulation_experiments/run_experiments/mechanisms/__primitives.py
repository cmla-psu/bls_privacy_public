...
def standard_gaussian_mechanism(prng, df, group_cols, agg_cols, std_list, identity=False):
    """ Applies the square root mechanism to df, returns a new data frame
    Do not call this function directly
    Input:
        prng: a numpy pseudorandom number generator
    ...
    for colname, scale in zip(agg_cols, std_list):
        stdname = std_col(colname)
        query_df[colname] = query_df[colname] + gaussian_noise(prng, scale, nrows)  # desired aggregate, sanitized
        query_df[stdname] = np.power(scale,2)  # std of aggregate, sanitized
    return query_df
