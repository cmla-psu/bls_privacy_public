...
class STANDGAUSAccountant(BaseAccountant):
    def make_noisy_measurements(self, identity_mu_dict, groupby_info):
        ...
        # groupby queries
        self.answer_identity_query_using_standgaus(identity_mu_dict)
        for query in groupby_info:
            self.answer_groupby_query_using_standgaus(query[GROUP], query[MU], name=query[NAME])
